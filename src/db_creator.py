...
def save_emp_data_to_database(data, database_name, params):
    conn = psycopg2.connect(dbname=database_name, **params)
...
def save_vac_data_to_database(data, database_name, params):
    conn = psycopg2.connect(dbname=database_name, **params)
    ...
    with conn.cursor() as cur:
        for vac in data:
            ...
            VALUES (%s, %s, %s, %s, %s)"""
            , (vac['id'], vac['employer']['id'], vac['name'], 20, vac['alternate_url'])
            )
    conn.commit()
    conn.close()
